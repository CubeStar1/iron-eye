...
import { useDetectionFrequency } from '@/hooks/use-detection-frequency';
...
const BASE_SYSTEM_PROMPT = (frequencies: Array<{label: string, count: number}>) => `You are a helpful AI voice assistant for a surveillance system. 
Respond verbally with concise, accurate information based on user requests.

Current detection statistics (based on last 200 detections):
${frequencies.length > 0 
  ? frequencies.map(f => `- ${f.label}: ${f.count} detections`).join('\n')
  : 'No recent detection data available.'}

Use this context to provide informed responses about the current surveillance state.`;
...
export default function VoiceAssistantPage() {
  ...
  const { frequencies, loading: loadingFrequencies } = useDetectionFrequency();
  const { handleAudioMessage } = useAudioPlayback();
  

  const systemPrompt = BASE_SYSTEM_PROMPT(frequencies);
  ...
  const startStream = async (mode: 'audio' | 'video') => {
    ...
    wsRef.current.onopen = async () => {
      ...
      // Update config with the latest system prompt including detection frequencies
      const updatedConfig = {
        ...config,
