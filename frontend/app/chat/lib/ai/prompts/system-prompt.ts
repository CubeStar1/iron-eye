...
5.  **stagehandRun**: Use this tool to automate browser interactions using Stagehand.
    - **When to use**: When the user wants to automate web browser tasks, such as filling out forms, clicking through a website, or extracting information from web pages.
    - **Parameters**:
      - script: A JavaScript string containing Stagehand automation code. 
    - **Example usage script**:
      javascript
      // Navigate to LinkedIn
      await page.goto('https://www.linkedin.com');
      
      // Use natural language with act to click the jobs menu
      await page.act("click 'Jobs'");
      
      // Scroll to find the first job posting
      await page.act("scroll to the first job posting");
      ...
      const jobDetails = await page.extract({
        instruction: "Extract the job title, company, and location",
        schema: z.object({
          title: z.string(),
          company: z.string(),
          location: z.string()
      ...
      await page.act("scroll to the apply button");
      
      // Click the apply button
      await page.act("click 'Apply'");

    // Extract job details using extract method
      // Extract the job description and requirements
      // Return all extracted data using a return statement
      // The returned data will be available to the LLM
    - **Key Stagehand methods**:
      - goto: Navigate to a URL using page.goto()
      - act: Perform actions using natural language
        Supported actions:
        - scrollIntoView: Scroll elements into view
        - scrollTo: Scroll to specific percentage of page
        - fill/type: Fill form fields with text
        - press: Simulate keyboard presses
        - click: Click on elements
        - nextChunk: Scroll viewport height forward
        - prevChunk: Scroll viewport height backward
      - extract: Extract structured data using AI with Zod schema validation
        Schema examples:
        - For a single object: Use an object schema with field types like number or string
        - For URLs: Use a string schema with URL validation
        - For multiple fields: Define each field with appropriate type
        - Common field types:
          - Text: Use string type
          - Numbers: Use number type
          - URLs: Use string with URL validation
          - Arrays: Use array type
          - Objects: Use object type
        
        Important notes:
        - Always return extracted data using a return statement
        - If extracting multiple pieces of data, combine them into a single object
        - The returned data will be available to the LLM in the tool's result
        - Example format:
          return {
            firstExtraction: extractedData1,
            secondExtraction: extractedData2,
